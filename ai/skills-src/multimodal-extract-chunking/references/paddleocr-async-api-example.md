# PaddleOCR 官方异步 API 参考代码

本文件只保存官方异步参考代码，供设计 `Extract Worker`、抽取适配层、PoC 脚本时对照使用。

不要把这里的代码片段重写成“仓库规范版本”后再当成官方事实引用。这里的目的就是保留官方接口原貌。

## 关键事实

- 提交地址：`https://paddleocr.aistudio-app.com/api/v2/ocr/jobs`
- 模型：`PaddleOCR-VL-1.5`
- 鉴权头：`Authorization: bearer {TOKEN}`
- 模式：
  - `fileUrl` URL 模式
  - `files={"file": f}` 本地文件上传模式
- 轮询：`GET https://paddleocr.aistudio-app.com/api/v2/ocr/jobs/{jobId}`
- 完成后取：`resultUrl.jsonUrl`
- `jsonUrl` 可能指向对象存储结果域，例如 `bj.bcebos.com`；不要把它和提交作业的主站域名混为一谈
- 官方异步返回本身是 JSONL，不是现成的 `.md`；仓库代码应从每个 `layoutParsingResults` 项的 `markdown.text` 组装拆分后的 Markdown 派生文件
