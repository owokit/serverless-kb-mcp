...
# EN: Repository root resolved two levels above this script.
# CN: 仓库根目录，从本脚本向上两级解析。
...
def main() -> int:
    ...
    _assert_reference_only(REPO_ROOT / "examples" / "workflows" / "workflow_reference_only" / "__init__.py", errors)
    _assert_reference_only(REPO_ROOT / "examples" / "workflows" / "workflow_reference_only" / "gemini_local_pipeline.py", errors)
    _assert_reference_only(REPO_ROOT / "examples" / "workflows" / "workflow_reference_only" / "openai_embedding_smoke.py", errors)
    _assert_reference_only(REPO_ROOT / "examples" / "workflows" / "workflow_reference_only" / "s3_vectors_check.py", errors)
    ...
    pytest_ini_path = REPO_ROOT / "services" / "pytest.ini"
...
def _assert_python_project_usage(text: str, filename: str, errors: list[str]) -> None:
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
        stripped = line.strip()
        if "uv sync --locked" in stripped and "--project services" not in stripped:
            errors.append(f"{filename}:{lineno} must run uv sync with --project services")
        if "uv run" in stripped:
            if "--no-project" in stripped or "--no-sync" in stripped:
                continue
            if "--project services" not in stripped:
                errors.append(f"{filename}:{lineno} must run uv with --project services")
...
def _assert_labeler_alignment(errors: list[str]) -> None:
    ...
    if "services/ocr-pipeline/**" not in labeler_text:
        errors.append(".github/labeler.yml must target services/ocr-pipeline/** for the service source tree")
...
def _assert_destroy_alignment(errors: list[str]) -> None:
    """EN: Ensure the destroy workflow reads the root pipeline config instead of the old scripts path.
    CN: 确保 destroy workflow 读取仓库根目录的 pipeline 配置，而不是旧的 scripts 路径。"""
    destroy_text = (REPO_ROOT / ".github" / "workflows" / "destroy.yml").read_text(encoding="utf-8")
    if "pipeline-config.json" not in destroy_text:
        errors.append(".github/workflows/destroy.yml must read pipeline-config.json from the repository root")
