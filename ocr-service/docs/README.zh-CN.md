...
1. 在运行 Python 测试或脚本前，先执行 `uv sync --locked --project ocr-service`。
2. 本地验证优先使用 `uv run --project ocr-service pytest -q`、`uv run --project ocr-service ruff check .` 和 `uv run --project ocr-service python ...`。
3. 将 `ocr-service/pyproject.toml` 和 `ocr-service/uv.lock` 视为 Python 依赖的唯一事实来源。
