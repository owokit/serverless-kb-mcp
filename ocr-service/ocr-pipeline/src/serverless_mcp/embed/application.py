...
class VersionCleanupService:
    ...
    def cleanup_previous_version_state(
        ...
        *,
    ...
    ) -> VectorCleanupPlan | None:
        """
        EN: Build a cleanup plan for the previous version instead of deleting vectors inline.
        CN: 鐢?cleanup plan 浠ｆ浛 Lambda 鍐呴儴鐩存帴鍒犻櫎鏃у悜閲忋€?
        """
        ...
        if previous_manifest_s3_uri is None and self._manifest_repo is not None:
            previous_manifest_s3_uri = self._manifest_repo.find_manifest_s3_uri(
                source=job.source,
                version_id=job.previous_version_id,
            )
...
class EmbedWorker:
    ...
    def __init__(
        ...
        *,
        ...
        cleanup_state_machine_arn: str | None = None,
        ...
        self._cleanup_state_machine_arn = cleanup_state_machine_arn
    ...
    def _handle_version_cleanup(self, *, job: EmbeddingJobMessage) -> None:
        ...
        """
        try:
            cleanup_plan = self._version_cleanup_service.cleanup_previous_version_state(job=job)
            if cleanup_plan is None:
                return
            self._start_cleanup_execution(cleanup_plan)
        except _EMBED_FAILURE_TYPES as exc:
            emit_trace(
    ...
    def _start_cleanup_execution(self, cleanup_plan: VectorCleanupPlan) -> None:
        """
        EN: Start the cleanup workflow with an idempotent execution name derived from the plan payload.
        CN: 浣跨敤鏉ヨ嚜 cleanup plan 鐨勭‘瀹氭€у悕绉版槸鍚姩 cleanup workflow銆?
        """
        if self._stepfunctions_client is None or not self._cleanup_state_machine_arn:
            raise ValueError("VECTOR_CLEANUP_STATE_MACHINE_ARN is required for previous-version cleanup")

        payload = {
            "cleanup_target": asdict(cleanup_plan),
    ...
    def _build_cleanup_execution_name(self, payload: dict[str, object]) -> str:
        """
        EN: Build a safe, deterministic Step Functions execution name for idempotent cleanup dispatch.
        CN: 涓轰簡鎸佷箙鍖栬皟搴︾敓鎴?Step Functions execution name銆?
        """
        digest = hashlib.sha256(json.dumps(payload, ensure_ascii=False, sort_keys=True).encode("utf-8")).hexdigest()
        return f"cleanup-{digest[:48]}"
