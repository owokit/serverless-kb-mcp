...
def test_load_vector_cleanup_state_machine_definition_renders_delete_vectors_task() -> None:
    ...
    assert parsed["States"]["DeleteVectors"]["Parameters"]["indexName.$"] == "$.cleanup_target.vector_index_name"
    assert parsed["States"]["DeleteVectors"]["Parameters"]["keys.$"] == "$.cleanup_target.keys"
    assert parsed["States"]["DeleteVectors"]["Retry"][0]["ErrorEquals"][0] == "S3Vectors.TooManyRequestsException"
    assert parsed["States"]["DeleteVectors"]["Catch"][0]["Next"] == "RecordCleanupFailure"
    assert parsed["States"]["RecordCleanupFailure"]["Resource"] == "arn:aws:states:::aws-sdk:dynamodb:updateItem"
    assert parsed["States"]["RecordCleanupFailure"]["Parameters"]["TableName"] == "${OBJECT_STATE_TABLE_NAME}"
    assert parsed["States"]["RecordCleanupFailure"]["Next"] == "FailCleanup"
    assert parsed["States"]["FailCleanup"]["Type"] == "Fail"
