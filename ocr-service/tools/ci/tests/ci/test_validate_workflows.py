...
MODULE_PATH = REPO_ROOT / "ocr-service" / "tools" / "ci" / "validate_workflows.py"
# EN: Path to the workflow-sanity.yml file used by inline assertions.
# CN: 这里是内联断言所用的 workflow-sanity.yml 路径。
SANITY_WORKFLOW_PATH = REPO_ROOT / ".github" / "workflows" / "workflow-sanity.yml"
...
def test_package_release_only_publishes_for_main_branch_workflow_runs() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "package-release.yml"
...
def test_failure_comment_relay_shell_step_passes_workflow_name() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "ci-failure-comment-relay.yml"
    text = workflow_path.read_text(encoding="utf-8")

    assert "uv run --project ocr-service python ocr-service/tools/ci/comment_pr_failure.py" in text
...
def test_failure_comment_workflows_request_pr_lookup_permissions() -> None:
    docs_workflow = REPO_ROOT / ".github" / "workflows" / "docs-ci.yml"
    relay_workflow = REPO_ROOT / ".github" / "workflows" / "ci-failure-comment-relay.yml"
...
def test_open_source_ci_strategy_documents_relay_section() -> None:
    ...
    assert "ocr-service/tools/ci/comment_pr_failure.py" in text
...
def test_labeler_tracks_the_renamed_workflow_reference_directory() -> None:
    ...
    assert "ocr-service/ocr-pipeline/**" in text
...
def test_codeql_tracks_the_real_js_ts_surfaces() -> None:
    codeql_path = REPO_ROOT / ".github" / "codeql" / "codeql-javascript-typescript.yml"
...
def test_guardrails_is_matrix_driven() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "guardrails.yml"
    text = workflow_path.read_text(encoding="utf-8")
    ...
    assert "ocr-service/tools/ci/chinese_text_hygiene.py" in text
...
def test_pr_path_conflict_guard_scans_parallel_prs() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "pr-path-conflict-guard.yml"
    text = workflow_path.read_text(encoding="utf-8")
...
def test_destroy_workflow_uses_root_pipeline_config() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "destroy.yml"
    text = workflow_path.read_text(encoding="utf-8")
...
def test_branch_lifecycle_cleanup_reconciles_labels_and_deletes_eligible_branches() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "merged-branch-cleanup.yml"
    text = workflow_path.read_text(encoding="utf-8")
...
def test_logic_ci_is_matrix_driven() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "logic-ci.yml"
    text = workflow_path.read_text(encoding="utf-8")
    ...
    assert "ruff check ocr-service/ocr-pipeline/src ocr-service/tools/ci ocr-service/ocr-pipeline/tests" in text
...
def test_contract_ci_is_matrix_driven() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "contract-ci.yml"
    text = workflow_path.read_text(encoding="utf-8")
    ...
    assert "ocr-service/ocr-pipeline/tests/unit/serverless_mcp/test_dynamo_batch.py" in text
...
def test_python_workflows_target_the_mcp_project_explicitly() -> None:
    ...
    required_snippets = {
        "docs-ci.yml": [
            ...
            "uv run --project ocr-service python ocr-service/tools/ci/comment_pr_failure.py",
        ],
        "workflow-sanity.yml": [
            "uv sync --locked --project ocr-service",
            "uv run --project ocr-service python -c \"$PYTHON_SCRIPT\"",
        ],
        "logic-ci.yml": [
            "uv sync --locked --project ocr-service",
            "uv run --project ocr-service ruff check ocr-service/ocr-pipeline/src ocr-service/tools/ci ocr-service/ocr-pipeline/tests",
            "uv run --project ocr-service python -m compileall ocr-service/ocr-pipeline/src ocr-service/tools/ci ocr-service/ocr-pipeline/tests",
        ...
        "security-ci.yml": [
            "uv sync --locked --project ocr-service",
            "uv run --project ocr-service python -m pip check",
            "uv run --project ocr-service --locked pip-audit",
            "uv run --project ocr-service python <<'PY'",
        ],
        "external-validation.yml": [
            "uv sync --locked --project ocr-service",
            "uv run --project ocr-service python -m pytest -q ocr-service/examples/workflows/tests/reference/test_gemini_local_pipeline.py --maxfail=1",
            "uv run --project ocr-service python -m pytest -q ocr-service/examples/workflows/tests/reference/test_s3_vectors_check.py --maxfail=1",
        ],
        "package-release.yml": [
            "uv sync --locked --project ocr-service",
            "uv run --project ocr-service python ./ocr-service/tools/packaging/serverless_mcp/list_lambda_artifacts.py",
...
def test_local_integration_ci_is_matrix_driven() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "local-integration-ci.yml"
    text = workflow_path.read_text(encoding="utf-8")
    ...
    assert 'str(repo_root / "ocr-service" / "ocr-pipeline")' in text
