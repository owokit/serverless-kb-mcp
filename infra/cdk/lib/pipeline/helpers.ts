...
export function renderVectorCleanupStateMachineDefinition(objectStateTableName: string): string {
  ...
  let template = fs.readFileSync(templatePath, 'utf8');
  template = template.replaceAll('${OBJECT_STATE_TABLE_NAME}', objectStateTableName);
  JSON.parse(template);
  return template;
}
