...
description: 说明组织级共享技能从 `skills-src/organization` 到 `.agents/.claude` 的生成约束，以及 `skills-src/organization/organization-skill-sync-policy/scripts/sync-ai.py` 的标准用法。
---
...
当你要新增、修改、删除组织级 skill，或者需要理解 skill 如何从 `skills-src/organization/` 同步到 `.agents/skills/` 和 `.claude/skills/` 时，使用这个 skill。当前组织级共享仓库本身就是模板与协作框架，因此所有技能文案都应围绕“维护源文件、模板、同步视图和发布约束”来写，而不是围绕某个单一产品功能来写。
...
- 仓库根目录的 `AGENTS.md`、`.github/ISSUE_TEMPLATE/README.md` 和 `.github/pull_request_template.md` 也是这个组织级框架的一部分，修改 skill 文案时要一起考虑它们的用词和约束是否一致。
- 不要在 skill 文案里硬编码任何具体 `owner/repo`；如果必须提到仓库身份，只能用当前仓库上下文或 `<owner>/<repo>` 占位符。
- fan-out 映射文件也应保持仓库中立；如果运行时需要知道来源仓库，交给 workflow 环境变量或脚本参数，不要写死在源数据里。
...
3. 如果修改的是组织级说明文件，先确认它是不是应该被 fan-out；像 `organization-skills-subtree-mappings.json` 这类元数据文件通常要排除。
4. 运行：
...
5. 确认 `.agents/skills/` 和 `.claude/skills/` 的生成结果与 `skills-src/organization/` 一致。
...
- 不要让技能文案和仓库内的模板、AGENTS、README、issue/PR 规则出现相互冲突的说法。
