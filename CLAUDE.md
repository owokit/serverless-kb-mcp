...
本仓库把主要项目规则拆分到独立 skill，`CLAUDE.md` 只保留总纲、结论和导航。
...
- 如果用户明确要求与现有设计或既有规则冲突，以用户当次明确要求为准；随后必须把冲突点同步更新到相关 skill 或 `CLAUDE.md`，包括这条冲突优先规则本身。

## Skill 导航

> Skills 维护在 `.claude/skills`（Claude Code 兼容）。

- github-issue-workflow
  - [.claude/skills/github-issue-workflow/SKILL.md](.claude/skills/github-issue-workflow/SKILL.md)
  - GitHub issue 优先工作流：先建 issue，再改代码；在技能入口提供简要说明、流程规则、评论规范和 PR 关联要求。
- github-cli
  - [.claude/skills/github-cli/SKILL.md](.claude/skills/github-cli/SKILL.md)
  - 面向 `gh` CLI 的通用 GitHub 操作入口，覆盖 issue、PR、分支、workflow/run 读取与评论写入。
- mainline-worktree
  - [.claude/skills/mainline-worktree/SKILL.md](.claude/skills/mainline-worktree/SKILL.md)
  - 适用于新建 worktree 前先同步本地 `main` 与 `origin/main`、创建任务分支和语义化命名。
- versioned-s3-ingest
  - [.claude/skills/versioned-s3-ingest/SKILL.md](.claude/skills/versioned-s3-ingest/SKILL.md)
  - 适用于 `S3 Versioning`、`S3 Event Notification`、`SQS ingest queue`、幂等、`object_state`、版本推进与旧版本治理。
- check-pr
  - [.claude/skills/check-pr/SKILL.md](.claude/skills/check-pr/SKILL.md)
  - 适用于 GitHub PR 的未解决评论、状态检查、描述完整性和回评修复。
- multimodal-extract-chunking
  - [.claude/skills/multimodal-extract-chunking/SKILL.md](.claude/skills/multimodal-extract-chunking/SKILL.md)
  - 适用于 OCR、版面分析、`Step Functions Standard` 轮询式异步抽取、PDF/PPT/图片切片、多模态 chunk 设计。
- vector-manifest-storage
  - [.claude/skills/vector-manifest-storage/SKILL.md](.claude/skills/vector-manifest-storage/SKILL.md)
  - 适用于 OpenAI Embedding / Azure OpenAI、`S3 Vectors` metadata、普通 `S3 manifest`、`DynamoDB` 状态与索引。
- project-delivery-guardrails
  - [.claude/skills/project-delivery-guardrails/SKILL.md](.claude/skills/project-delivery-guardrails/SKILL.md)
  - 适用于仓库结构、分层边界、实施顺序、跨层改动审查。
- durable-s3-embed-pipeline
  - [.claude/skills/durable-s3-embed-pipeline/SKILL.md](.claude/skills/durable-s3-embed-pipeline/SKILL.md)
...
- python-bilingual-comments
  - [.claude/skills/python-bilingual-comments/SKILL.md](.claude/skills/python-bilingual-comments/SKILL.md)
  - 适用于当前 Python 项目的模块 docstring、类注释、函数注释、方法注释、块注释和行内注释双语规范，要求英文一行、中文一行逐行对应。
- docs-mcp-router
  - [.claude/skills/docs-mcp-router/SKILL.md](.claude/skills/docs-mcp-router/SKILL.md)
  - 将 AWS、Azure、Google、GitHub、Cloudflare、OpenAI 和 Anthropic 的文档问题路由到对应的官方 MCP 或第一方文档来源。
- bilingual-programming
  - [.claude/skills/bilingual-programming/SKILL.md](.claude/skills/bilingual-programming/SKILL.md)
...
- 如果 `CLAUDE.md` 与 skill 冲突，先统一设计，再同步更新两边，不要长期分叉。
- 仓库内新增或更新的文档必须使用简体中文。
...
- 如果引入新的 embedding provider、embedding profile、vector bucket/index 或查询融合策略，必须同步更新 `CLAUDE.md`、相关 skill、`docs/` 手动部署文档和测试。
