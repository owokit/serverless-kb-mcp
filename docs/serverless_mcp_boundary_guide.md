...
本文记录 `services/ocr-pipeline/src/serverless_mcp` 当前的主要边界，便于后续继续拆分时对齐职责。
...
- `serverless_mcp/server/`：远程 MCP handler 使用的 FastMCP 实现和传输安全设置。
- `serverless_mcp/entrypoints/`：Lambda / API 入口层。
